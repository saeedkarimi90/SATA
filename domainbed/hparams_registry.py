...
def _hparams(algorithm, dataset, random_state):
    ...
    hparams["optimizer"] = ("adam", "adam")      #adam, sgd replaced

    hparams["freeze_bn"] = (True, True) #true, true
    hparams["pretrained"] = (True, True)  # only for ResNet

    if dataset not in SMALL_IMAGES:
        hparams["lr"] = (5e-5, 10 ** random_state.uniform(-5, -3.5)) # -3 instead of -5
        if dataset == "DomainNet":
            hparams["batch_size"] = (16, int(2 ** random_state.uniform(3, 5)))  #32
        else:
            hparams["batch_size"] = (16, int(2 ** random_state.uniform(3, 5.5))) #16 instead 32
        if algorithm == "ARM":
    ...
    if dataset in SMALL_IMAGES:
        ...
        hparams["weight_decay"] = (0.0, 10 ** random_state.uniform(-6, -2)) #0.0
