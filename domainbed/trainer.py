...
def train(test_envs, args, hparams, n_steps, checkpoint_freq, logger, writer, target_env=None):
    ...
    for step in range(n_steps):
        ...
        if step % 200 == 0 and step >= 200: # or step % 200 == (200 - 1) :  #erm  checkpoint_freq ---> 400
            results = {
                "step": step,
            ...
            algorithmTTA = sata.SATA( alg, alg.optimizer,steps = 1)
            accuracies, summaries, losses = evaluator.evaluate(algorithmTTA, TTA = True)
    
            print('SATA_loss =', losses)
            print('SATA_acc = ', accuracies)
